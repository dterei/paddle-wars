...
public class Interface extends JApplet implements ActionListener,
    ...
    public void init() {
        ...
        desktopWidth = this.getSize().width;
        desktopHeight = this.getSize().height;
    ...
    public void loadSounds() {
        ...
        try {
            gameSounds[0] = getAudioClip(getCodeBase(),
                    "Resources/Sounds/rub_bounce.wav");
            gameSounds[1] = getAudioClip(getCodeBase(),
                    "Resources/Sounds/bonus.wav");
            gameSounds[2] = getAudioClip(getCodeBase(),
                    "Resources/Sounds/explode-1.wav");
            gameSounds[3] = getAudioClip(getCodeBase(),
                    "Resources/Sounds/metal_bounce.wav");
            gameSounds[4] = getAudioClip(getCodeBase(),
                    "Resources/Sounds/explode-0.wav");
        } catch (Exception e) {
    ...
    public void loadImageIcon() {
        try {
            ...
            slimeurl = new URL(getDocumentBase(),
                    "Resources/Pics/SlimeBall.png");
            yellowpaddleurl = new URL(getDocumentBase(),
                    "Resources/Pics/Paddles/paddle-2-1.gif");
            greenpaddleurl = new URL(getDocumentBase(),
                    "Resources/Pics/Paddles/paddle-0-1.gif");
            bluepaddleurl = new URL(getDocumentBase(),
                    "Resources/Pics/Paddles/paddle-1-1.gif");
    ...
    public void loadImages() {
        try {
            backGround = getImage(getCodeBase(),
                    "Resources/Pics/backGround.gif");
            for (int i = 0; i < 12; i++) {
                explosion[i] = getImage(getCodeBase(),
                        "Resources/Pics/Explosion/Explosion-" + (i + 1)
                                + ".gif");
                prepareImage(explosion[i], this);
            }
            for (int i = 0; i < 12; i++) {
                BallImages[i] = getImage(getCodeBase(),
                        "Resources/Pics/Balls/Ball-" + balls + "-" + (i + 1)
                                + ".gif");
                prepareImage(BallImages[i], this);
            }
            for (int i = 0; i < 4; i++) {
                lightningBall[0][i] = getImage(getCodeBase(),
                        "Resources/Pics/lightningBall/lightning-0-" + (i + 1)
                                + ".gif");
                lightningBall[1][i] = getImage(getCodeBase(),
                        "Resources/Pics/lightningBall/lightning-1-" + (i + 1)
                                + ".gif");
                prepareImage(lightningBall[0][i], this);
                prepareImage(lightningBall[1][i], this);
            }
            for (int i = 0; i < 4; i++) {
                PaddleImage[0][i] = getImage(getCodeBase(),
                        "Resources/Pics/Paddles/paddle-" + paddles[0] + "-"
                                + (i + 1) + ".gif");
                PaddleImage[1][i] = getImage(getCodeBase(),
                        "Resources/Pics/Paddles/paddle-" + paddles[1] + "-"
                                + (i + 1) + ".gif");
    ...
    public void newWindow() {
        newFrame = new JInternalFrame("Choose New Game type...", false, //resizable
                true, //closable
                false, //maximizable
                false); //iconifiable
        ...
        newFrame.setBounds(0, desktopHeight / 2 - newFrameHeight / 2,
                desktopWidth, newFrameHeight);
    ...
    public void optionsWindow() {
        ...
        jPanel2 = new JPanel();
        jPanel1 = new JPanel();
        optionsFrame = new JInternalFrame("Options", true, //resizable
                true, //closable
                true, //maximizable
                true); //iconifiable
        ...
        optionsFrame.setBounds(desktopWidth / 2 - optFrameWidth / 2,
                desktopHeight / 2 - optFrameHeight / 2, optFrameWidth,
                optFrameHeight);
        ...
        OptTab.setBounds(-2, 0, optFrameWidth, optFrameHeight);
    ...
    public void paddleWarsSetup(int gameMode) {
        ...
        loadImages();
        loadSounds();
        paddleWars = new PaddleWars((int) (jFrameSizeX * 0.99),
                (int) (jFrameSizeY * 0.89), gameSounds, gameMode);
    ...
    public static void setMetalBorders() {
        try {
            UIManager
                    .setLookAndFeel("UIManager.com.sun.javax.swing.plaf.basic.BasicInternalFrameUI");
        } catch (Exception e) {
