...
/**
 * This is the main clas for the paddle Wars game. It creates and controls all the other classes, paddle, ball, bonusManager. It also controls event handling and AI.
 *
 * @see paddle
 * @see ball
 * @see bonusManager
...
public class PaddleWars extends JPanel implements Runnable {
    ...
    /** Stores the animation image for an explosion.
     */
    //private Image explosion;
    /**
     * Stores th three diferent paddle images.
     */
    private Image[][] PaddleImages;
    /**
     * Stores the image of the fireball, used for the ball.
     */
    private Image[] FireBall;
    /**
     * Stores the images for the lightning ball animation, for both ways.
     */
    private Image[][] LightningBall;
    /**
     * The images which make up the explosion animation when a missile hits a paddle.
     */
    private Image[] Explosion;
     ...
     * @param height The height of the area in which the game should
     *     operate.
     * @param gameSounds All the sounds needed for the game.
     * @param backGround The background image of the game.
     * @param Paddles All the paddle Images possible.
     * @param fireBall The images needed for the fireball animation.
     * @param lightningBall The images needed for the lightning ball
     *     animation.
     * @param explosion The explosion image.
     * @param gameMode The current game mode.
     */
    public PaddleWars(int width, int height, AudioClip[] gameSounds, Image backGround,
                          Image[][] Paddles, Image[] fireBall,
                          Image[][] lightningBall, Image[] explosion,
                          int gameMode) {
    ...
    public void start() {
        ...
        Ball[0] = new ball(aWidth/2,aHeight/2,5,FireBall,aWidth,aHeight, WallHit[0]);

        Ball[1] = new ball(aWidth/2,aHeight/2,5,BonusColor,BonusType,aWidth,aHeight, WallHit[0]);
        ...
        for (int i=0; i<MissileLength;i++) {
            Missiles[0][i] = new ball(aWidth/2,aHeight/2,5,LightningBall[0],aWidth,aHeight,WallHit[1],i,0);
            Missiles[0][i].setBallVelocityX(0);
            Missiles[0][i].setBallVelocityY(0);
            Missiles[0][i].increaseSize(-2);
            Missiles[1][i] = new ball(aWidth/2,aHeight/2,5,LightningBall[1],aWidth,aHeight,WallHit[1],i,1);
            Missiles[1][i].setBallVelocityX(0);
            Missiles[1][i].setBallVelocityY(0);
            Missiles[1][i].increaseSize(-2);
        }

        Paddle[0] = new paddle(Px[0],(aHeight-PSizeY[0])/2,PSizeX[0],PSizeY[0],PaddleImages[0], Explosion, aHeight, Gspeed, 0, PaddleSounds[0]);
        Paddle[1] = new paddle(Px[1],(aHeight-PSizeY[1])/2,PSizeX[1],PSizeY[1],PaddleImages[1], Explosion, aHeight, Gspeed, 1, PaddleSounds[1]);
    ...
    public void paint(Graphics g) {
        ...
        //create a new colour using RGB-A
        ...
        //test if the bonus ball is running
        ...
        if (paused == true) {
            ...
            g2.drawString("PAUSED",(int)(aWidth*0.45),(int)(aHeight*0.51));
            //g2.drawImage(explosion,300,300,this);
        }

        if (gameOver == 1) {
            g2.setColor(Color.RED);
            g2.drawString(winMsg,(int)(aWidth*0.45),(int)(aHeight*0.51));
        }
     ...
     *
     * @param acessMode If you want to set if a player has the hyper key, or
     *     just find out if they do or not.
     * @param paddle Which paddle you want to perform the operations
     *     on.
     * @param giveHyperKey ive them the Hyper Key or not, 0= not,
     *     1=yes. Only relevant if acessMode = 1.
     ...
     * @param increasePercent The percent by which to increase the
     *     hyper movement value.
    ...
    private void FireMissile(int player, int missile) {
        ...
        Missiles[player][missile] = new ball(MissileX,MissileY,5,LightningBall[player],aWidth,aHeight,WallHit[1],missile,player);
        ...
        public void keyPressed(KeyEvent e) {
            ...
            if (keyStatus[key] == 0) {
                ...
                if (keyStatus[pB[0]] == 1) {
                    ...
                    if (key == pU[0]) { PyM[0] += -pMvB[0]; }
                    if (key == pD[0]) { PyM[0] += pMvB[0]; }
                ...
                if (keyStatus[pB[1]] == 1) {
                    ...
                    if (key == pU[1]) { PyM[1] += -pMvB[1]; }
                    if (key == pD[1]) { PyM[1] += pMvB[1]; }
                ...
                if (key == pB[0]) {
                    ...
                    if (keyStatus[pU[0]] == 1) { PyM[0] += -pMvB[0]; }
                    if (keyStatus[pD[0]] == 1) { PyM[0] += pMvB[0];  }
                ...
                if (key == pB[1]) {
                    ...
                    //check wat keys are ALREADY down, and reaply their movement
                    //with the Hyper value
                    if (keyStatus[pU[1]] == 1) { PyM[1] += -pMvB[1]; }
                    if (keyStatus[pD[1]] == 1) { PyM[1] += pMvB[1];  }
            ...
            if (GotM[ThisPaddle] == 1 && Stunned[ThisPaddle] == 0
                ...
                if (PyM[OtherPaddle] < 0) {
                    ...
                    if (PaddleTe < PaddleB+10 && PaddleTe > PaddleM+5) {
                        ...
                        AImissileDelay[1] = 25 * (int)(Math.round(100/Gspeed));
                    }
                ...
                if (PyM[OtherPaddle] == 0) {
                    ...
                    if (PaddleM > PaddleTe && PaddleM < PaddleBe) {
                        MissileHandler(ThisPaddle);
                        AImissileDelay[ThisPaddle] = 25 * (int)(Math.round(100/Gspeed));
                    }
